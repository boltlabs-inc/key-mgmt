//////
///
/// This module defines a [`PasswordKey`], which can be used to securely encrypt
/// passwords.
use super::generic::{AssociatedData, EncryptionKey};
...
pub(super) mod test {
    ...
    fn password_key_from_bytes() -> Result<(), LockKeeperError> {
        ...
        assert_eq!(
            ...
            password_encryption_key_from_bytes.0
        );
    ...
    fn password_key_from_file() -> Result<(), LockKeeperError> {
        ...
        password_encryption_key_file
            .write_all(&password_encryption_key_bytes)
        ...
        let password_encryption_key_from_file =
            PasswordKey::read_from_file(PASSWORD_KEY_FILENAME).unwrap();
        let password_encryption_key_from_file_bytes =
            password_encryption_key_from_file.0.clone().into_bytes();
        ...
        // make sure that password key in the file is the same as the original password
        // key
        assert_eq!(
            password_encryption_key_from_file_bytes,
            password_encryption_key_bytes
