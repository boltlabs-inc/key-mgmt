...
pub type ServerFuture = JoinHandle<Result<(), lock_keeper_key_server::LockKeeperServerError>>;
...
async fn start_server(server_config: ServerConfig) -> ServerFuture {
    ...
    let server_handle = tokio::spawn(
        lock_keeper_key_server::server::start_lock_keeper_server(server_config)
...
pub fn get_logs(log_type: LogType, party: Party) -> Result<String, anyhow::Error> {
    ...
    Ok(logs
        .lines()
        .filter(|s| s.contains("lock_keeper_key_server::"))
