...
/// A well-formed symmetric encryption key.
///
/// This key is for [ChaCha20Poly1305](https://www.rfc-editor.org/rfc/rfc8439),
/// an authenticated encryption with associated data (AEAD) scheme, using the
...
#[allow(unused)]
impl EncryptionKey {
    /// Generate a new ChaCha20Poly1305 encryption key from scratch.
...
where
    ...
    Vec<u8>: From<T>,
{
    /// Encrypt the `T` under the [`EncryptionKey`] with the [`AssociatedData`].
    ///
...
/// The master key is a default-length symmetric key for the ChaCha20Poly1305
/// AEAD scheme.
///
...
#[allow(unused)]
impl MasterKey {
    /// Encrypt the given [`StorageKey`] under the [`MasterKey`], using the
    /// ChaCha20Poly1305 AEAD scheme.
...
/// A storage key is a default-length symmetric key for the ChaCha20Poly1305
/// AEAD scheme. It is used to encrypt stored secrets.
///
