...
/// The [`LockKeeperRpc`] trait is autogenerated based on `protoc` file.
///
/// It is then up to us to implement this trait. The methods you see implemented below are never
/// called directly by us. Instead they are called server-side as gRPC endpoints when the client
/// object makes a request.
///
/// All these methods follow a similar pattern:
/// 1) Receive a request from client. This request contains the `receiver` end of a channel.
/// 2) The method creates a MPSC channel. It uses the `receiver` end from step #1 and the `sender`
/// end from this step to create a Channel.
