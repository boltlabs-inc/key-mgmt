...
use lock_keeper::transaction::TransactionApprovalRequest;
...
pub struct PolicyEngineConfig {
    ...
    /// The number of [`SelfCustodial`](lock_keeper::keys::SelfCustodial) keys
    /// each user can create.
    #[serde(default)]
    max_self_custodial: u16,
    /// The number of [`Delegated`](lock_keeper::keys::Delegated) keys each user
    /// can create.
    #[serde(default)]
    max_delegated: u16,
...
/// [`SharedControl`](lock_keeper::keys::SharedControl).
///
